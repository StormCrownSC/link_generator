...
func IsShortLinkUniquePostgres(shortLink string) bool {
	db, err := connectToDB()
	if err != nil {
...
func SaveLinkMappingPostgres(shortLink, originalLink string) error {
	db, err := connectToDB()
	if err != nil {
		fmt.Println("Failed to connect to the database:", err)
		return err
...
func IsOriginalLinkExistsPostgres(originalLink string) (string, bool, error) {
...
func GetOriginalLinkPostgres(shortLink string) (string, error) {
	// Устанавливаем соединение с базой данных
	db, err := connectToDB()
	if err != nil {
